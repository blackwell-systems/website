...
var limax_exports = {};
__export(limax_exports, {
  default: () => limax
});
module.exports = __toCommonJS(limax_exports);
var import_speakingurl = __toESM(require("speakingurl"), 1);
var import_hepburn = __toESM(require("hepburn"), 1);
...
function limax(text, opt) {
  ...
  if (lang.toLowerCase().startsWith("ja") || import_hepburn.default.containsKana(text)) {
    text = import_hepburn.default.fromKana(text);
    text = text.replace(/([^A-Za-z0-9\- ]+)/g, "");
  } else if (import_pinyin_pro.pinyin && (lang.toLowerCase().startsWith("zh") || /[\u4e00-\u9fa5]+/.test(text))) {
    const tone = typeof options.tone === "boolean" ? options.tone : true;
    mergeDigitSuffixes = tone;
    const data = (0, import_pinyin_pro.pinyin)(text, { type: "all", toneType: "none", nonZh: "consecutive" }).map((item) => {
      if (item.isZh) {
  ...
  const slug = (0, import_speakingurl.default)(deburredText, {
    lang: lang || "en",
